...
- **GPT windows are found by length, not by slot.** `chatgpt.com/backend-api/wham/usage` is undocumented; the session window is whichever of `rate_limit.primary_window` / `secondary_window` has `limit_window_seconds == 18000`, weekly is `604800`. A missing weekly window warns, and so does a window of any other length. A missing session window does **not**: since July 2026 some plans (seen on `plan_type: "prolite"`) return only the weekly window in `primary_window` with `secondary_window: null`. `session_percent` is then `None`, serialized as `null`, and the popup drops the Session row (the tray draws `S --`). Per-model limits come from `additional_rate_limits[]` (e.g. `GPT-5.3-Codex-Spark`): each is a separate limit with its own windows, shown under Models by its weekly window, like Claude's `weekly_scoped` entries. The test body in `gpt_parser.rs` is hand-written, not captured — replace it with a neutralised live capture in `fixtures/` when one exists.
